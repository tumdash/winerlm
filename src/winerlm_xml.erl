...
parse_response(XmlResponse) ->
	{Xml, _} = xmerl_scan:string(binary_to_list(XmlResponse)),
	EnumParse = xmerl_xpath:string(
		"//s:Envelope/s:Body/n:EnumerateResponse/n:EnumerationContext/text()",
		Xml
	),
...
parse_type(#xmlElement{name='n:PullResponse'} = Xml, _XmlResponse) ->
	...
	{pull_resp, FinalMap};
parse_type(#xmlElement{name=Unknown} = Xml, _XmlResponse) ->
	{error, Unknown}.
